...
pprint(f"Start of boozerLS surface 1")

# Directory for output
OUT_DIR = f"./output_ls1/"
os.makedirs(OUT_DIR, exist_ok=True)
pprint("================================")

try:
    [coils, axis, problem_config, _, _] = load(f"./example_input_7241.json")
except:
    pprint("not found")
    exit(1)
...
qsc = Qsc(ma.rc, np.insert(ma.zs, 0, 0), nfp=nfp, etabar=etabar)
full_torus = SurfaceRZFourier.from_pyQSC(qsc, 0.05, ntheta=100, mpol=5, ntor=5)
...
mpol, ntor = 2, 2
surface = SurfaceXYZTensorFourier(mpol=mpol, ntor=ntor, stellsym=stellsym, nfp=nfp, quadpoints_phi=phis, quadpoints_theta=thetas)
surface.least_squares_fit(full_period.gamma())

iota0 = qsc.iota
G0 = 2. * np.pi * sum(abs(c.current.get_value()) for c in coils) * (4 * np.pi * 10**(-7) / (2 * np.pi)) 
is_inter = np.any([surface.is_self_intersecting(a) for a in np.linspace(0, 2*np.pi, 10)])
pprint("initial surface ", is_inter, iota0, G0, surface.major_radius(), surface.aspect_ratio(), surface.volume())

init_surfaces = [surface]
init_iotaG = [(iota0, G0, 1.)]
vol_list = [2*np.pi**2 * 1. * 0.05**2 * np.sign(surface.volume())]
surfaces, boozer_surfaces = run_surface_continuation(init_surfaces, init_iotaG, vol_list)

init_surfaces = surfaces
init_iotaG = [(bs.res['iota'], bs.res['G'], 1.) for bs in boozer_surfaces]
pprint((surfaces[0].mpol, surfaces[0].ntor), surfaces[0].aspect_ratio(), surfaces[0].major_radius(), surfaces[0].volume())
...
KAPPA_THRESHOLD = 5.
MSC_THRESHOLD = 5.
LENGTH_THRESHOLD = problem_config['clen_target'] * len(base_curves)
MR_TARGET = 1.
